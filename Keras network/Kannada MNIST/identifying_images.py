...
from train_models import transform_image, contrastive_loss, compute_accuracy, get_data
...
def create_positive_pairs(x, digit_indices, test_with_transformations=False):
    pairs = []
    labels = []
    indices = []
...
def place_in_array(array, dig_idx, index):
    for j in range(len(dig_idx)):
        if j == 9:
            ...
            break
        elif np.any(dig_idx[j] <= index) and np.any(dig_idx[j+1] > index):
...
def test_tp_fn(model, plot_location_tp_fn, digits_location_tp_fn, transformations_enabled):
    ...
    te_digits_indices = [np.where(y_test == i)[0] for i in range(num_classes)]
    
    for i in range(len(te_digits_indices)):
        print(te_digits_indices[i].shape)
    te_pairs, te_y, dig_idx = create_positive_pairs(x_test, te_digits_indices, test_with_transformations=transformations_enabled)
...
def test_tn_fp(model, plot_location_tn_fp, digits_location_tn_fp, transformations_enabled):
    ...
    te_negative_pairs, te_y, dig_idx = create_negative_pairs(x_test, te_digits_indices, test_with_transformations=transformations_enabled)
