...
from keras.metrics import binary_accuracy, mean_squared_error, mean_absolute_error, mean_absolute_percentage_error
...
def get_data():
    x_train = np.load(kmnist_train_images_file)['arr_0']
    y_train = np.load(kmnist_train_labels_file)['arr_0']

    x_test = np.load(kmnist_test_images_file)['arr_0']
    y_test = np.load(kmnist_test_labels_file)['arr_0']
...
def prepare_data_for_work(transformations, split_and_save_data):
    ...
    if split_and_save_data:
        save_data('x_training_data_part_', tr_pairs)
        save_data('y_training_data_part_', tr_y)
        save_data('x_test_data_part_', te_pairs)
        save_data('y_test_data_part_', te_y)
